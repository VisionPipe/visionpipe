...
pub fn png_file_to_data_uri(path: &str) -> Result<String, Box<dyn std::error::Error>> {
...
/// Capture a specific window by its CGWindowID using macOS `screencapture -l`.
/// Returns a full Retina-resolution PNG as a base64 data URI.
pub fn capture_window(window_id: u32) -> Result<String, Box<dyn std::error::Error>> {
    let tmp = "/tmp/visionpipe-capture.png";

    let status = Command::new("screencapture")
        .args(["-x", "-r", "-l", &window_id.to_string(), tmp])
        .status()?;

    if !status.success() {
        return Err(format!("screencapture failed for window ID {}", window_id).into());
    }
