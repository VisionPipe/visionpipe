...
function App() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const headerFont = "bold 14px Verdana, Geneva, sans-serif";
    const bodyFont = "14px Verdana, Geneva, sans-serif";
    const monoFont = "12px 'Source Code Pro', 'Courier New', monospace";
    const lineHeight = 22;
    ...
    panelLines.push({ text: "LLM Instructions: This is a screenshot generated by VisionPipe.ai", font: headerFont, color: C.amber });
    panelLines.push({ text: "for you to evaluate. Here is what the user had to say about the", font: headerFont, color: C.amber });
    panelLines.push({ text: "screenshot above:", font: headerFont, color: C.amber });
    panelLines.push({ text: "", font: bodyFont, color: C.cream }); // spacer

    // User comments — word-wrap at ~70 chars
    const words = userCommentText.split(/\s+/);
    let currentLine = "";
    for (const word of words) {
      if ((currentLine + " " + word).length > 70 && currentLine) {
        panelLines.push({ text: currentLine, font: bodyFont, color: C.cream });
        currentLine = word;
      } else {
        currentLine = currentLine ? currentLine + " " + word : word;
      }
    }
    if (currentLine) panelLines.push({ text: currentLine, font: bodyFont, color: C.cream });
    panelLines.push({ text: "", font: bodyFont, color: C.cream }); // spacer

    // Instruction for unclear input
    panelLines.push({ text: "If you do not understand the user instructions then do your best to", font: bodyFont, color: C.textMuted });
    panelLines.push({ text: "understand it and provide the user back with your recommendations", font: bodyFont, color: C.textMuted });
    panelLines.push({ text: "based on what you find.", font: bodyFont, color: C.textMuted });
    panelLines.push({ text: "", font: bodyFont, color: C.cream }); // spacer

    // Metadata
    panelLines.push({ text: `app: ${metadata.app} | window: ${metadata.window}`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: `os: ${metadata.os} (${metadata.osBuild}) | uptime: ${metadata.uptime}`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: `display: ${metadata.resolution} @ ${metadata.scale} | ${metadata.primaryDisplay} (${metadata.displayCount} display${metadata.displayCount !== 1 ? "s" : ""})`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: `cpu: ${metadata.cpu} | memory: ${metadata.memoryGb}`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: `user: ${metadata.username}@${metadata.hostname} | locale: ${metadata.locale} | tz: ${metadata.timezone}`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: `color space: ${metadata.colorSpace} | dark mode: ${metadata.darkMode ? "yes" : "no"} | battery: ${metadata.battery}`, font: bodyFont, color: C.textMuted });
    if (metadata.activeUrl) {
      panelLines.push({ text: `url: ${metadata.activeUrl}`, font: bodyFont, color: C.textMuted });
    }
    panelLines.push({ text: `captured: ${metadata.timestamp} | region: ${metadata.captureWidth}x${metadata.captureHeight}`, font: bodyFont, color: C.textMuted });
    panelLines.push({ text: "", font: bodyFont, color: C.textMuted }); // spacer
    panelLines.push({ text: `VisionPipe v0.1.0`, font: bodyFont, color: C.textMuted });
